...
class ReportGenerator:
    def __init__(self, config):
        self.config = config
        self.out_dir = Path(config['output']['base_dir']) / "reports"
    ...
    def build_srt(self, video_stem, events):
        """Generates a standard SubRip (.srt) file for video players like VLC."""
        srt_path = self.out_dir / f"events_{video_stem}.srt"
        ...
        with open(srt_path, 'w', encoding='utf-8') as f:
            for idx, evt in enumerate(sorted_events):
                ...
                # Show subtitles starting 1.5s before and ending 3.5s after the event
    ...
    def build_html(self, video_name, duration, report_json_path):
        """Generates a premium, browser-viewable HTML report showing complete analytics."""
        with open(report_json_path, 'r') as f:
            data = json.load(f)

        html_path = self.out_dir / f"report_{Path(video_name).stem}.html"
        
        # Format events into HTML list
        events_html = ""
        for e in data.get('events', []):
            ...
            <div class="card" style="border-left: 4px solid {sev_color};">
                ...
                <div class="row" style="margin-top: 5px; font-size: 0.9em; color: #ADB5BD;">
                    Confidence: {e.get('confidence', 0.0):.2%}; Location/Details: {e.get('bbox', 'N/A')}
                </div>
            </div>
            """

        # Format people into HTML list
        people_html = ""
        ...
        # Format chapters into HTML list
        chapters_html = ""
        ...
        # Build threat score timeline SVG chart
...
<body>
    <header>
        ...
        <div style="text-align: right;">
            ...
            <div style="color: #8C98A4; margin-top: 5px;">Fusion Mode: {data.get('fusion_mode', 'visual_only')}</div>
        </div>
    </header>
    ...
    <div class="grid" style="grid-template-columns: repeat(4, 1fr); gap: 20px; margin-bottom: 30px;">
        <div class="metric-box">
            ...
            <div class="metric-value">{Path(video_name).name}</div>
        </div>
...
"""
    ...
    def build_json(self, video_name, duration, aggregated_results):
        """Generates the main structured JSON report file."""
        # Calculate maximum threat level for metadata
        ...
        out_path = self.out_dir / f"report_{timestamp}.json"
