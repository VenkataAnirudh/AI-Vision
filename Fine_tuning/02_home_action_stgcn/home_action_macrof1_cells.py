# ============================================================================
#  NB 02 - ADD-ON CELLS  (paste these as TWO new cells at the BOTTOM of
#  home_action_stgcn_colab.ipynb, AFTER the existing Train + Evaluate cells)
#
#  WHAT THEY DO
#    Re-run the same Stratified K-Fold CV, but pick the best fold by MACRO-F1
#    (best selector for imbalanced / safety-critical falls) instead of val_loss,
#    then save to a SEPARATE file: home_action_stgcn_best_macrof1.pt
#    Your original home_action_stgcn_best.pt is left untouched.
#
#  REQUIREMENTS
...
#    STGCN, SkeletonDataset, alert_idx, device, WORK_DIR and all hyperparams
#    (SEED, N_SPLITS, PATIENCE, EPOCHS, LR, WEIGHT_DECAY, BATCH_SIZE, etc.).
#
#  COST
...
macrof1_save_path = os.path.join(WORK_DIR, "home_action_stgcn_best_macrof1.pt")
...
torch.save({"model_state_dict": g_best_state, "class_names": CLASS_NAMES,
    ...
    "config": {"clip_len": CLIP_LEN, "max_persons": MAX_PERSONS, "num_joints": NUM_JOINTS,
        ...
        "pose_model": POSE_MODEL, "pose_conf": POSE_CONF}}, macrof1_save_path)
...
print("%-26s %6s %6s %6s %6s" % ("class", "prec", "rec", "f1", "n"))
f1s = []
for i, c in enumerate(CLASS_NAMES):
    ...
    print("%-26s %6.3f %6.3f %6.3f %6d%s" % (c, prec, rec, f1, cm[i, :].sum(), flag))
